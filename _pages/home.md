...
header:
  ...
  cover_letters:
    - label: "🇺🇸 English"
      ...
      class: "btn--light-outline btn--medium"
    - label: "🇩🇪 Deutsch"
...
<script>
// Intro Display Control with sessionStorage (브라우저 닫으면 리셋)
document.addEventListener('DOMContentLoaded', function() {
    const INTRO_STORAGE_KEY = 'homepage-intro-viewed';

    // intro 섹션 찾기
    const introSection = document.querySelector('section:has(.researcher-intro)') ||
                        document.querySelector('.researcher-intro')?.closest('section') ||
                        document.querySelector('.feature__wrapper .feature__item .researcher-intro')?.closest('.feature__wrapper');

    if (introSection) {
        // sessionStorage에서 intro 표시 여부 확인 (브라우저 세션 동안만 유지)
        const introViewed = sessionStorage.getItem(INTRO_STORAGE_KEY);
        ...
        window.addEventListener('scroll', () => {
            ...
            scrollTimeout = setTimeout(() => {
                if (window.scrollY > 100) { // 100px 이상 스크롤시
                    recordInteraction();
                }
            }, 100);
        });
        ...
        // 키보드 입력 감지 (ESC 키나 아무 키나)
        document.addEventListener('keydown', (e) => {
            recordInteraction();
        });

        // 5초 후 자동으로 기록 (사용자가 읽을 시간을 준 후)
        setTimeout(() => {
            recordInteraction();
        }, 5000);

        // intro 섹션에 닫기 버튼 추가 (옵션)
        const closeButton = document.createElement('button');
        closeButton.innerHTML = '×';
        closeButton.className = 'intro-close-btn';
        closeButton.setAttribute('aria-label', 'Close intro');
