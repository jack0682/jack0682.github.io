...
$$\mathcal{M} = \{(p_i, c_i) \mid p_i \in \mathbb{R}^3, c_i \in \mathcal{C}\}$$
...
- $$E = \{(v_i, r_{ij}, v_j) \mid \text{Cond}(v_i, v_j)\}$$: edges with relational types
...
$$\mathcal{L}_{\text{pred}} = \sum_i \lVert\hat{\mathcal{S}}_i(t+1) - \mathcal{S}_i(t+1)\rVert^2$$
...
where:
- $$\lvert\Delta f_c\rvert \leq \alpha\|\Delta G\|$$: crossover frequency drift due to compensator insertion
...
where $$\lvert\Delta f_c\rvert \leq \alpha\|\Delta G\|$$, $$\epsilon \geq 0$$ accounts for uncertainties, and $$\sigma > 0$$ is a safety buffer.
...
$$\|\Phi_{\sigma_1}(f) - \Phi_{\sigma_2}(f)\|_\infty \leq \frac{\lvert\sigma_1 - \sigma_2\rvert}{\min(\sigma_1, \sigma_2)} \|f\|_{\operatorname{Lip}}$$
...
1. **Semantic Energy**: $$F(\mathcal{S}) = \frac{1}{2}\lVert\mathcal{S}\rVert^2$$ gives energy evolution
2. **Distance Dynamics**: $$F(\mathcal{S}_1, \mathcal{S}_2) = \lVert\mathcal{S}_1 - \mathcal{S}_2\rVert^2$$ for relative semantics
...
For small perturbations $$\lVert\Delta A\rVert \ll \lVert A\rVert$$:
...
$$\lvert\lambda_i(\mathcal{L}_n) - \lambda_i(\mathcal{L})\rvert \leq \|\mathcal{L}_n - \mathcal{L}\|_{\operatorname{op}}$$
...
$$\max_i \lvert\lambda_i(\mathcal{L}_n) - \lambda_i(\mathcal{L})\rvert \to 0$$
...
$$\langle\mathcal{S}_1 - \mathcal{S}_2, \nabla\mathcal{L}(\mathcal{S}_2) - \nabla\mathcal{L}(\mathcal{S}_1)\rangle \leq L\lVert\mathcal{S}_1 - \mathcal{S}_2\rVert^2$$

*Step 3: Differential Inequality*

$$\frac{d}{dt}\lVert\mathcal{S}_1(t) - \mathcal{S}_2(t)\rVert^2 \leq 2L\lVert\mathcal{S}_1(t) - \mathcal{S}_2(t)\rVert^2$$

*Step 4: Gronwall Application*

$$\lVert\mathcal{S}_1(t) - \mathcal{S}_2(t)\rVert^2 \leq e^{2Lt}\lVert\mathcal{S}_1(0) - \mathcal{S}_2(0)\rVert^2$$

*Step 5: Flow Map Lipschitz Continuity*

$$\lVert\Phi_t(\mathcal{S}_1) - \Phi_t(\mathcal{S}_2)\rVert \leq e^{Lt}\lVert\mathcal{S}_1 - \mathcal{S}_2\rVert$$
...
$$\leq \lvert t_2 - t_1\rvert \sup_{s \in [t_1, t_2]} \lVert\nabla\mathcal{L}(\Phi_s(\mathcal{S}))\rVert \leq M\lvert t_2 - t_1\rvert$$

for bounded gradient $$M$$.

*Step 7: Combined Hölder Bound*

$$\lVert\Phi_{t_2}(\mathcal{S}_1) - \Phi_{t_1}(\mathcal{S}_2)\rVert \leq e^{Lt_{\max}}\|\mathcal{S}_1 - \mathcal{S}_2\|^{\alpha} + M\lvert t_2 - t_1\rvert^{\alpha}$$
...
$$\lim_{t \to \infty} \frac{1}{t} \log\lVert v(t)\rVert = \lambda_i$$
...
$$\frac{1}{t} \log\lvert\det(\Phi_t'(\mathcal{S}))\rvert \to \sum_{i=1}^d \lambda_i$$

*Step 3: Kaplan-Yorke Conjecture*

The Lyapunov dimension is:
$$D_L = j + \frac{\sum_{i=1}^j \lambda_i}{\lvert\lambda_{j+1}\rvert}$$
