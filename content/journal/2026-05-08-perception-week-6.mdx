...
<Figure
  ...
  caption={<>Fig. 25. Same scene, four observers, four percepts. Each panel shows the optimal cohesion field u* on the same 8×8 grid under a different observer weight vector (λ_cl, λ_sep, λ_bd, λ_tr). The diagnostic bars (Bind, Sep, Inside, Persist) quantify what each observer prioritises. Closure-dominant observers produce concentrated fields (Bind=0.93, Sep=0.45); separation-dominant observers produce spread fields (Bind=0.84, Sep=0.94). OMS-2.0 proves this is not a gauge redundancy — it is structure.</>}
...
<Figure
  ...
  caption={<>Fig. 26. Observer branch map — Δ²_static projection. Each point is an observer weight vector (λ_cl, λ_sep, λ_bd) evaluated on scene P12; color encodes the branch class (core-count signature). Seven distinct branches emerge from 165 evaluation points. The dominant branch (3,4) covers 64.2% of the simplex; transition edges account for 31.1% of adjacencies — well within the codim-1 budget of 37.5%. VP-10 result (Session 6).</>}
