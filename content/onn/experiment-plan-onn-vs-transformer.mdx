...
kind: essay
summary: "A Feb-2026 pre-registered protocol for a constraint-satisfaction (CSR/ASR/SVR) benchmark of an ONN LOGOS solver vs. Transformer baselines. Kept as a methodology record; its sibling results docs are not reflected in the current authoritative research source."
...
> **Note (2026-07-10).** This is a *plan* — a pre-registered protocol, with
> **target** thresholds (e.g. "CSR ≥ 95%"), not measured results. It belongs to
> a Feb-2026 constraint-satisfaction benchmark line (a LOGOS constraint-solver
> vs. Transformer comparison) whose asserted *results* live in the
> [claim ledger](/onn/claim-ledger-ral/) and
> [logic lock](/onn/logic-lock-onn-vs-transformer/) — and those results **do not
> appear in the current authoritative research source** (`onn_ws/ONN`). The ONN
> programme's central question later resolved to a scoped **No-Go boundary**, not
> to this positive benchmark. Read this page as methodology only; for the audited
> state see the [research status](/onn/onn-research-status-2026/).
