...
<Figure
  ...
  caption={<>Closure iteration u, Cl(u), Cl²(u), … converges to a unique fixed point at geometric rate a_cl/4 in the contraction regime (a_cl below 4). The trajectory carries structural information; the destination is unique (CN1, CN9 — two-landscape structure).</>}
...
<Figure
  ...
  alt="Two-panel diagram: graded diagnostic vector (4 horizontal bars) on the left, Boolean projection collapsed to a single FALSE box on the right."
  caption={<>The proto-cohesion diagnostic d = (Bind, Sep, Inside, Persist) ∈ [0,1]^4. The graded vector preserves four-dimensional information about which conditions are weakly satisfied; the Boolean projection collapses this to a single bit, losing diagnostic discrimination.</>}
  className="mx-auto max-w-full"
/>
