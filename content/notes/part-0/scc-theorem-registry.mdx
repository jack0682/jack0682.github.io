...
track: perception
updated: 2026-04-27
summary: "The authoritative index of all proved SCC theorems — 43 Category A + 4 Category B + 5 Category C + 5 retracted (57 claims, 75% fully proved). Aligned with Perception_theory canonical CV-1.5 (W5 Day 1 G0, 2026-04-27)."
...
**Structure:** Rows are organized by canonical version (CV-1.0 → CV-1.5; **current = CV-1.5**) then status (active, resolved, challenged, deprecated).
...
<Figure
  ...
  caption={<>The 9 SCC hero theorems among the 43 Cat A entries below, organized by foundation, phase + stability, W4 capstone, and W5 Day 1 G0 σ-framework supporting structures. See /notes/scc-heroes/ for the curated index.</>}
...
### Canonical Spec v1.5 (2026-04-27) — Current Version (W5 Day 1 G0: σ-Framework Supporting Structures)
...
| **T-σ-Theorem-4** | σ at First Pitchfork on $D_4$ Free-BC Grid (Leading Order) | accepted | A in $\epsilon$-small regime | C-0716 | P-0716 | (theoretical via T-Birth-Parametric + R22 axis-aligned) | $D_4 \to \mathbb{Z}_2$ symmetry breaking; Mode 0 trivial irrep / Mode 1 sign irrep split; $K_0 = K_1$ leading-order degeneracy ($A_2/A_1 = 4$); $\mathcal{F}$ tie-break NQ-184 |
