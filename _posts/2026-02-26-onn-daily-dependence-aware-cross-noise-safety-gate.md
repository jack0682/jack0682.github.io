...
Target: Dependence-aware cross-noise safety gate for `b_H` and `L_cp^0.95`.
...
For each tracked noise family `nu`, require conservative coverage certification for `b_H(n_win, nu)` through dependence-corrected lower bounds `LB_nu(alpha_H)`. In parallel, require CP lead-time safety through dependent-bootstrap confidence intervals on `L_cp^0.95(nu)`. Accept only when both guards pass:
- coverage guard: `LB_nu(alpha_H) >= 1 - alpha_H`
- lead-time guard: `max_nu CI_cp^0.95(nu) <= L_max`
...
- optimistic `n_eff` can invalidate coverage guarantees,
...
- Replaced IID-style counts with conservative `n_eff` from thinning/block-bootstrap policy.
...
- Validate CI lower coverage under ARFIMA/fGn stress with conservative `n_eff` choices.
...
- Extend certification logs to enforce schema checks for `(nu, n_eff, LB_nu, L_cp^0.95, CI_cp^0.95, delta_kappa)`.
