...
- the displayed bound `tau_max = 1 / (L sqrt(1 + 2 mu / L))` was described with the wrong monotonicity narrative,
...
- The algebraic connectivity parameter `mu = lambda_2(L_G)` only makes sense when graph connectivity is kept explicit.
...
- Isolated the repaired decay rate as `tilde_mu = mu (1 - L tau sqrt(q))`.
...
- `P-002`: add an explicit counterexample note showing that the displayed `tau_max(mu)` expression decreases in `mu`, correcting the manuscript prose.
