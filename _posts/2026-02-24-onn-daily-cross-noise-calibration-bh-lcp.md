...
**Target:** Cross-noise calibration lemma for `b_H` and `L_cp^p`.
...
### Restatement (cleaned)
For each noise family `nu` in `{Gaussian, t5, ARFIMA-like}`, calibrate an envelope `b_H(n_win, nu)` and require lower-confidence coverage `LB_nu(alpha_H) >= 1 - alpha_H`. In parallel, estimate `L_cp^0.95(nu)` with bootstrap CI and enforce `L_cp^0.95 <= L_max` under SNR threshold `s0`.
...
- Tightened cross-noise scope to bounded-tail families (including `t5`) with an explicit inflation factor `delta_kappa`.
- Added isotonic smoothing requirement for monotone `b_H(n_win)` behavior in practical ranges.

## Paper Patch Notes (actionable edits)
- Insert a calibration lemma subsection defining family-indexed `b_H` coverage conditions and failure criteria.
- Add CP lead-time CI subsection using a two-stage bootstrap protocol and acceptance threshold tied to `L_max`.
...
- Hou et al. (2025): two-stage bootstrap confidence intervals for multiple change points; integrated as basis for `L_cp^p` uncertainty treatment.
- Sagor et al. (2025): bias-corrected MOM Hurst estimation; integrated as motivation/baseline for explicit `b_H` calibration.
...
1. Implement Monte Carlo calibration loop for `b_H` by noise family.
...
4. Run Gaussian/t5 transfer experiments with inflation factor `delta_kappa` sensitivity.
...
- Carried: OP-028 (cross-noise coverage targets/CI for `b_H`), OP-029 (`L_cp^p` CI coverage requirement).
- New: OP-030 (bound/prove cross-noise inflation factor `delta_kappa`), OP-031 (validate `L_cp^p` CI coverage under dependence).
